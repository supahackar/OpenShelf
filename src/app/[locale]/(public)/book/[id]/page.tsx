...
export default async function BookPage({ params }: BookPageProps) {
    ...
    const listingT = await getTranslations("NewListing")
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="grid gap-4 rounded-lg border p-4 text-sm">
                        <div className="flex items-center gap-2">
                            ...
                            <span>{t("donatedBy", { name: listing.donor.name || "" })}</span>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span>{listing.createdAt ? t("listedAgo", { time: formatDistanceToNow(listing.createdAt) }) : ""}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="font-semibold">{t("condition")}:</span>
                            <span>{listingT(`conditions.${listing.condition}`)}</span>
                        </div>
                        {listing.courseCode && (
                            <div className="flex items-center gap-2">
                                <span className="font-semibold">{t("courseCode")}:</span>
                                <span>{listing.courseCode}</span>
                            </div>
                        )}
                        {listing.edition && (
                            <div className="flex items-center gap-2">
                                <span className="font-semibold">{t("edition")}:</span>
